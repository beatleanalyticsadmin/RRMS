...
const ComplaintsDetailsUser = ({ navigation, route }) => {
  ...
  const fetchComplaintDetails = async () => {
    ...
    try {
      const response = await fetch(
        `${dataCenter.apiUrl}/lm/complaint_details.php`,
        {
          method: "POST",
          headers: {
  ...
  const updateComplaintStatus = async () => {
    ...
    try {
      ...
      const apiUrl = `${dataCenter.apiUrl}/lm/resolve_complaint.php`;
              ...
              <View style={styles.requestContainer}>
                <View style={styles.header}>
                  {complaint.guest_photo &&
                    complaint.guest_photo.length > 0 && (
                      <Image
                        source={{
...
export default ComplaintsDetailsUser;
